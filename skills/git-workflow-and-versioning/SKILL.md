...
### AI-Augmented Trunk-Flow (Recommended)

This strategy combines the safety and release-control of **Git Flow** with the speed, automation, and continuous integration of **Trunk-Based Development**, optimized for teams operating with **AI Agents**.
...
gitGraph
    ...
    commit id: "Agent: Create schema"
    commit id: "Agent: Add routes"
    checkout develop
...
- **Short-lived feature branches.** Feature branches target `develop` and must live for no more than 1-2 days to prevent code drift and ease AI merge conflict resolution.
- **Automated AI Gates.** Before code enters `develop`, automated AI-linter, test suite, and semantic validations must pass.
...
*   **`develop` (Active Trunk)**: The integration branch for developers and AI agents. Always stable and deployable to staging.
...
Maintain strict branch naming so automation/AI systems can route and validate appropriately:
...
# Agents/Developers can work in parallel without interfering
ls ../
  project/              ← develop branch (integration workspace)
  project-feature-a/    ← feature/task-creation branch (agent sandbox A)
  project-feature-b/    ← feature/user-settings branch (agent sandbox B)
